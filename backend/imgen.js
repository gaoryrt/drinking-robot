...
const saveImages = async (
  ...
  try {
    ...
    const outputPath = path.join(outputDir, `${formattedDate}.jpeg`);
...
export const generateImageFiles = async (prompt) => {
  ...
  const session = createSession(authCookie);
  const imageLinks = await getImages(session, prompt);
  const filename = await saveImages(session, imageLinks, outputDir);
  ...
  return {
    filename,
    md5:CryptoJS.MD5(fileData).toString(),
    data: fileData.toString("base64"),
  };
  // const image = imageFiles.map((filename) => {
  // });
